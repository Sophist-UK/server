	...
	const dict_table_t*		table)
{
	if (error_key_num == ULINT_UNDEFINED) {
		return(FTS_DOC_ID_INDEX_NAME_ls.str);
	...
	bool			check_only=false)
{
	...
	*num_v = 0;
	...
	for (i = 0; i < n_cols; i++) {
		...
		if (!field->field_name.streq(FTS_DOC_ID_COL_NAME_ls)) {
			continue;
		}

		if (strcmp(field->field_name.str, FTS_DOC_ID_COL_NAME_ls.str)) {
	...
	for (; i + DATA_N_SYS_COLS < (uint) table->n_cols; i++) {
		...
		if (strcmp(name, FTS_DOC_ID_COL_NAME_ls.str) == 0) {
#ifdef UNIV_DEBUG
	...
	*/
	...
	if (fulltext_indexes && m_prebuilt->table->fts) {
		...
		for (Field** fp = table->field; *fp; fp++) {
			...
			if ((*fp)->field_name.streq(FTS_DOC_ID_COL_NAME_ls)) {
				ha_alter_info->unsupported_reason = my_get_err_msg(
					ER_ALTER_OPERATION_NOT_SUPPORTED_REASON_CHANGE_FTS);
				DBUG_RETURN(HA_ALTER_INPLACE_NOT_SUPPORTED);
			}
	...
	for (Create_field& cf : ha_alter_info->alter_info->create_list) {
		...
		if (const Field* f = cf.field) {
			...
			if (fulltext_indexes > 1
			    && (*af)->field_name.streq(FTS_DOC_ID_COL_NAME_ls)) {
						...
						ha_alter_info */
{
	...
	if (altered_table) {
		...
		for (uint i = 0; i < altered_table->s->keys; i++) {
			...
			if (!key.name.streq(FTS_DOC_ID_INDEX_NAME_ls)) {
				continue;
			}
			...
			if ((key.flags & HA_NOSAME)
			    ...
			    && !(key.key_part[0].key_part_flag
				 & HA_REVERSE_SORT)
			    && !strcmp(key.name.str, FTS_DOC_ID_INDEX_NAME_ls.str)
			    && !strcmp(key.key_part[0].field->field_name.str,
				       FTS_DOC_ID_COL_NAME_ls.str)) {
				if (fts_doc_col_no) {
					*fts_doc_col_no = ULINT_UNDEFINED;
				}
	...
	for (index = dict_table_get_first_index(table);
		...
		if (index->type & DICT_CORRUPT
		    || !index_name.streq(FTS_DOC_ID_INDEX_NAME_ls)) {
			continue;
		}

		if (!dict_index_is_unique(index)
		    || dict_index_get_n_unique(index) != table->fts_n_uniq()
		    || strcmp(index->name, FTS_DOC_ID_INDEX_NAME_ls.str)) {
			return(FTS_INCORRECT_DOC_ID_INDEX);
		}
		...
		if (strcmp(field->name, FTS_DOC_ID_COL_NAME_ls.str) == 0
		    && !field->descending
			...
			/*!< in: MySQL table that is being altered */
{
	...
	if (rebuild) {
		...
		if (new_primary) {
			...
			index->name = innobase_index_reserve_name_ls.str;
			index->rebuild = true;
			index->key_number = ~0U;
...
created_clustered:
	...
	if (add_fts_doc_idx) {
		...
		index->name = FTS_DOC_ID_INDEX_NAME_ls.str;
		...
		if (ctx->need_rebuild()) {
			...
			ctx->new_table->fts_doc_id_index
				= dict_table_get_index_on_name(
					ctx->new_table, FTS_DOC_ID_INDEX_NAME_ls.str);
		...
		if (innobase_fulltext_exist(table)
			...
			for (uint i = 0; i < table->s->keys; i++) {
				if (table->key_info[i].name.
				     streq(FTS_DOC_ID_INDEX_NAME_ls)) {
