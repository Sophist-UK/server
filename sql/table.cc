...
bool Lex_ident_fs::check_table_name(bool disallow_path_chars) const
{
  if (!disallow_path_chars &&
      (disallow_path_chars= check_mysql50_prefix(str)))
  {
    return check_body(str + MYSQL50_TABLE_NAME_PREFIX_LENGTH,
                      length - MYSQL50_TABLE_NAME_PREFIX_LENGTH,
                      disallow_path_chars);
  }

  return check_body(str, length, disallow_path_chars);
}
