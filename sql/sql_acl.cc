...
bool sp_revoke_privileges(THD *thd,
                          const LEX_CSTRING &sp_db,
                          const LEX_CSTRING &sp_name,
                          const Sp_handler *sph)
{
  uint counter, revoked;
  int result;
  ...
  do
  {
    for (counter= 0, revoked= 0 ; counter < hash->records ; )
    {
      GRANT_NAME *grant_proc= (GRANT_NAME*) my_hash_element(hash, counter);
      // QQ: a bug was here?
      if (Lex_ident_db(Lex_cstring_strlen(grant_proc->db)).streq(sp_db) &&
          // QQ: does not match to mysql.proc.name collation
          Lex_ident_routine(Lex_cstring_strlen(grant_proc->tname)).
            streq(sp_name))
      {
        LEX_USER lex_user;
