  ...
  */
...
#else
  {
    ...
    */
    ...
    if (Lex_ident_fs(table_name).check_table_name(false))
    {
...
absent:
  ...
  */
  ...
  if (sp_result != SP_KEY_NOT_FOUND &&
      ...
      sp_revoke_privileges(thd, lex->spname->m_db, lex->spname->m_name,
                           Sp_handler::handler(lex->sql_command)))
  {
    push_warning(thd, Sql_condition::WARN_LEVEL_WARN,
                                             ...
                                             LEX_STRING *option)
{
  ...
  if (!MY_TEST(table_options & TL_OPTION_ALIAS) &&
      unlikely(Lex_ident_fs(table->table).check_table_name(false)))
  {
  ...
  if (lower_case_table_names)
  {
    if (!(ptr->table_name= Lex_ident_table(thd->make_ident_casedn(
                                             ptr->table_name))).str)
      DBUG_RETURN(0);
    if (ptr->db.length && ptr->db.str != any_db.str &&
        !(ptr->db= Lex_ident_db(thd->make_ident_casedn(ptr->db))).str)
      DBUG_RETURN(0);
  }
